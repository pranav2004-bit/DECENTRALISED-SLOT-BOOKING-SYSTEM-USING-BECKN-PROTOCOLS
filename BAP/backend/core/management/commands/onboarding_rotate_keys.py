...
class Command(BaseCommand):
    help = (
        "Rotates BAP's signing+encryption key pairs and re-Subscribes an already-"
        "SUBSCRIBED domain with the new keys, per livetracker1.md 3.4 "
        "('re-Subscribe with new key_pair before valid_until'). "
        "Generates the new keys in memory and submits them to Registry BEFORE ever "
        "writing them to disk — the old key stays live on disk throughout the "
        "Subscribe attempt (Registry requires the re-Subscribe's Authorization header "
        "to be signed with the CURRENTLY REGISTERED key, not the new one, see "
        "onboarding_service.submit_subscribe's own docstring). Only once Registry "
        "actually confirms the new identity are the new keys persisted. If Subscribe "
        "fails, disk is never touched — no rollback needed, because nothing was ever "
        "written. Safe to invoke on any schedule — by default this only actually "
        "rotates once the signing key is at least settings.KEY_ROTATION_DAYS old; pass "
        "--force to rotate immediately regardless of age."
    )
    ...
    def handle(self, *args, **options):
        ...
        # Registry's own synchronous handling of POST /subscribe calls back to BAP TWICE
        # before it ever returns a response: once to fetch the domain-ownership
        ...
        # not just one. See onboarding_service's own module-level comment for why this
        # can't just be an in-process variable.
        ...
        try:
            status = onboarding_service.submit_subscribe(
                options["domain"],
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Re-Subscribed {status.domain} with rotated keys: {status.status}"
            )
        )
