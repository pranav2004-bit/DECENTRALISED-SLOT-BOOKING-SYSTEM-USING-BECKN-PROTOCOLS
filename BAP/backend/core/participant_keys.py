...
applied to Registry's `registry_keys.py` §1.2, Gateway's `participant_keys.py` §2.2, and
BPP's own copy) — the same bug applies here: BAP runs multiple gunicorn/daphne workers,
and an in-process cache means a real rotation writes the new key to disk but every
...
of the live workers actually serving traffic) — it never touched the workers' own stale
caches. Not a hot path here either (participant-key reads happen at Subscribe/dispatch
time, not every request), so re-reading a small JSON file per call is cheap and
correctness-first.
"""
...
def rotate_signing_key() -> tuple[str, str]:
    ...
    logger.warning("Rotated BAP signing key (previous key backed up to %s).", backup_path)
    return public_key, private_key
...
def rotate_encryption_key() -> tuple[str, str]:
    ...
    logger.warning("Rotated BAP encryption key (previous key backed up to %s).", backup_path)
    return public_key, private_key
...
def restore_keys_from_backup(backup_paths: dict[str, str]) -> None:
    """Rolls BAP's own identity back to what a prior `backup_current_keys()` call saved
    — used when a rotation's re-Subscribe step fails, so BAP doesn't end up signing with
    ...
    logger.warning("Restored BAP's signing+encryption keys from backup after a failed rotation.")
