...
def _mock_bpp_registry_lookup(rsps, *, bpp_id="bpp.example.com", url="https://bpp.example.com"):
    """livetracker4.md §1.1: every direct trigger_init call now does a fresh
...
def test_trigger_init_targets_the_same_bpp_from_selection_and_sends_a_real_signed_order(
    ...
    selection reaches the BPP directly (livetracker4.md §1.1 — no Gateway hop for
    /init anymore) as a real, signed Beckn Order targeting the exact same BPP that
    /select already resolved to — the customer never supplies bpp_id directly,
    and the resent order drops the stale quote (the BPP recomputes it fresh)."""
...
@pytest.mark.django_db
def test_on_init_view_accepts_missing_gateway_authorization_now_that_gateway_is_optional(client):
    """livetracker4.md §1.2: on_init now arrives directly from the BPP with no
...
@pytest.mark.django_db
def test_on_init_view_rejects_missing_bpp_authorization_even_without_gateway(client):
