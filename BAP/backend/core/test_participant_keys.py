...
BPP's `core/test_select.py::test_concurrent_select_on_the_same_slot_yields_exactly_one_winner`
failing intermittently in full-suite runs — same shared bug pattern, independent
copy of this module). Calls `_load_or_generate()` directly, bypassing `lru_cache`,
so this test exercises the actual race window rather than whatever `lru_cache`'s
own internal locking happens to mask on a given run.
"""
