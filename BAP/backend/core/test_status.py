"""Phase 3.5 Test Gate (livetracker2.md §3.5) pieces owned by BAP: real /status
trigger (customer-facing, non-Beckn shape, targets the same BPP a prior
successful /confirm already resolved to), the result poll, and real /on_status
...
def _build_on_status_payload(*, bap_id="bap-backend.local", bpp_id="bpp.example.com", error=None):
    payload = {
        "context": {
            "domain": "ONDC:RET13",
        ...
        "message": {
            "order": {
                ...
                "status": "ACTIVE",
                "fulfillments": [
                    {"id": "booking-1", "state": {"descriptor": {"code": "SCHEDULED"}}}
                ],
            }
        },
    }
...
def test_trigger_status_targets_the_same_bpp_from_confirm_and_sends_a_real_signed_order_id(
    ...
    assert forwarded["context"]["action"] == "status"
    assert forwarded["message"] == {"order_id": "booking-1"}
    assert "Authorization" in captured_requests[0].headers


@pytest.mark.django_db
def test_trigger_status_rejects_when_bpp_no_longer_subscribed(bap_identity_settings, client):
    """livetracker4.md §1.4 coverage-parity replacement for beckn-gateway's retired
    test_dispatch_status_does_not_forward_when_bpp_no_longer_subscribed — Gateway no
    longer re-checks SUBSCRIBED status for /status, so BAP's own
...
@pytest.mark.django_db
def test_trigger_status_view_rejects_a_transaction_with_no_confirmed_booking(
    bap_identity_settings, client
):
...
@pytest.mark.django_db
def test_status_result_view_returns_404_for_an_unknown_transaction(client):
    resp = client.get(reverse("status-result", args=["nonexistent-txn"]))
    assert resp.status_code == 404


@pytest.mark.django_db
def test_status_result_view_returns_the_recorded_order(client):
    session = _session_with_confirmed_order()
    session.status_order = {"status": "ACTIVE"}
    session.save()

    resp = client.get(reverse("status-result", args=["txn-1"]))
    assert resp.status_code == 200
    body = resp.json()
    assert body["status_order"] == session.status_order
    assert body["status_error"] is None


@pytest.mark.django_db
@patch("core.status_service.record_on_status_result")
def test_on_status_view_acks_when_both_bpp_and_gateway_signatures_are_valid(mock_record, client):
...
@pytest.mark.django_db
def test_record_on_status_result_stores_the_real_fulfillment_state():
    _session_with_confirmed_order()
    payload = _build_on_status_payload()
    ...
    assert session.status_order["fulfillments"][0]["state"]["descriptor"]["code"] == "SCHEDULED"
    assert session.status_error is None


@pytest.mark.django_db
def test_record_on_status_result_stores_a_real_error_instead_of_an_order():
    _session_with_confirmed_order()
    payload = _build_on_status_payload(
    ...
    session = SearchSession.objects.get(transaction_id="txn-1")
    assert session.status_error == {
        "code": "SLOT_UNAVAILABLE",
        "message": "No matching booking for this order",
    }
    assert session.status_order is None


@pytest.mark.django_db
def test_record_on_status_result_drops_a_callback_for_an_unknown_transaction():
    payload = _build_on_status_payload()
