"""Phase 4.5 Test Gate (livetracker2.md §4.5) pieces owned by BAP: real /support
trigger (customer-facing, non-Beckn shape, targets the same BPP a prior
successful /confirm already resolved to), the result poll, and real /on_support
...
def _build_on_support_payload(*, bap_id="bap-backend.local", bpp_id="bpp.example.com", error=None):
    payload = {
        "context": {
            "domain": "ONDC:RET13",
...
def _mock_bpp_registry_lookup(rsps, *, bpp_id="bpp.example.com", url="https://bpp.example.com"):
    """livetracker4.md §1.1: every direct trigger_support call now does a fresh
...
def test_trigger_support_targets_the_same_bpp_from_confirm_and_sends_a_real_signed_ref_id(
    ...
    assert forwarded["context"]["action"] == "support"
    assert forwarded["message"]["support"]["ref_id"] == "booking-1"
    assert "Authorization" in captured_requests[0].headers


@pytest.mark.django_db
def test_trigger_support_rejects_when_bpp_no_longer_subscribed(bap_identity_settings, client):
    """livetracker4.md §1.4 coverage-parity replacement for beckn-gateway's retired
    test_dispatch_support_does_not_forward_when_bpp_no_longer_subscribed — Gateway no
    longer re-checks SUBSCRIBED status for /support, so BAP's own
...
@pytest.mark.django_db
def test_trigger_support_view_rejects_a_transaction_with_no_confirmed_booking(
    bap_identity_settings, client
):
...
@pytest.mark.django_db
def test_support_result_view_returns_404_for_an_unknown_transaction(client):
    resp = client.get(reverse("support-result", args=["nonexistent-txn"]))
    assert resp.status_code == 404


@pytest.mark.django_db
def test_support_result_view_returns_the_recorded_result(client):
    session = _session_with_confirmed_order()
    session.support_result = {"ref_id": "booking-1", "email": "owner@example.com"}
    session.save()

    resp = client.get(reverse("support-result", args=["txn-1"]))
    assert resp.status_code == 200
    body = resp.json()
    assert body["support_result"] == {"ref_id": "booking-1", "email": "owner@example.com"}
    assert body["support_error"] is None


@pytest.mark.django_db
def test_support_trigger_view_rejects_a_different_authenticated_customers_booking(
    bap_identity_settings, client
):
    """SEC (§3.7's own Test Gate): an authenticated customer attempting to
    request support for another customer's booking is rejected with 403, not
...
@pytest.mark.django_db
@patch("core.support_service.record_on_support_result")
def test_on_support_view_acks_when_both_bpp_and_gateway_signatures_are_valid(mock_record, client):
    bpp_pub, bpp_priv = generate_signing_key_pair()
    gateway_pub, gateway_priv = generate_signing_key_pair()
    payload = _build_on_support_payload()
...
@pytest.mark.django_db
def test_on_support_view_accepts_missing_gateway_authorization_now_that_gateway_is_optional(
    client,
):
    """livetracker4.md §1.2: on_support now arrives directly from the BPP with no
...
@pytest.mark.django_db
def test_on_support_view_rejects_missing_bpp_authorization_even_without_gateway(client):
...
@pytest.mark.django_db
def test_record_on_support_result_stores_the_real_contact_info():
    _session_with_confirmed_order()
    payload = _build_on_support_payload()
    ...
    assert session.support_result == {"ref_id": "booking-1", "email": "owner@example.com"}
    assert session.support_error is None


@pytest.mark.django_db
def test_record_on_support_result_stores_a_real_error_instead_of_a_result():
    _session_with_confirmed_order()
    payload = _build_on_support_payload(
    ...
    session = SearchSession.objects.get(transaction_id="txn-1")
    assert session.support_error == {
        "code": "SLOT_UNAVAILABLE",
        "message": "No matching booking for this order",
    }
    assert session.support_result is None


@pytest.mark.django_db
def test_record_on_support_result_drops_a_callback_for_an_unknown_transaction():
    payload = _build_on_support_payload()
