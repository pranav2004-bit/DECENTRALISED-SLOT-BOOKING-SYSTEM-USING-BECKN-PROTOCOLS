...
def test_trigger_support_targets_the_same_bpp_from_confirm_and_sends_a_real_signed_ref_id(
    ...
    def gateway_support_callback(request):
    ...
    with responses.RequestsMock() as rsps:
        rsps.add_callback(
            responses.POST, "http://gateway:8000/support", callback=gateway_support_callback
        )
        resp = client.post(
...
@pytest.mark.django_db
def test_on_support_view_rejects_missing_gateway_authorization(client):
    bpp_pub, bpp_priv = generate_signing_key_pair()
    payload = _build_on_support_payload()
    body = json.dumps(payload).encode()
    bpp_header = sign_outbound_request(
