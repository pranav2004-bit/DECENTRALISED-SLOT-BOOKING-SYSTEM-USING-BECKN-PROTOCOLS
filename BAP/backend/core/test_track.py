"""Phase 3.5 Test Gate (livetracker2.md §3.5) pieces owned by BAP: real /track
trigger (customer-facing, non-Beckn shape, targets the same BPP a prior
successful /confirm already resolved to), the result poll, and real /on_track
receipt (verifying both the BPP and the forwarding Gateway). `/on_track`'s
message carries `tracking`, not `order` — §L.4/§L.5's own confirmed wire shape.
"""
...
def _build_on_track_payload(*, bap_id="bap-backend.local", bpp_id="bpp.example.com", error=None):
    payload = {
        "context": {
            "domain": "ONDC:RET13",
...
def _mock_bpp_registry_lookup(rsps, *, bpp_id="bpp.example.com", url="https://bpp.example.com"):
    """livetracker4.md §1.1: every direct trigger_track call now does a fresh
...
def test_trigger_track_targets_the_same_bpp_and_sends_a_real_signed_order_id(
    ...
    assert forwarded["context"]["action"] == "track"
...
@pytest.mark.django_db
def test_track_result_view_returns_404_for_an_unknown_transaction(client):
    resp = client.get(reverse("track-result", args=["nonexistent-txn"]))
    assert resp.status_code == 404


@pytest.mark.django_db
def test_track_result_view_returns_the_recorded_tracking_object(client):
    session = _session_with_confirmed_order()
    session.tracking = {"status": "inactive"}
    session.save()

    resp = client.get(reverse("track-result", args=["txn-1"]))
    assert resp.status_code == 200
    body = resp.json()
    assert body["tracking"] == session.tracking
    assert body["tracking_error"] is None


@pytest.mark.django_db
@patch("core.track_service.record_on_track_result")
def test_on_track_view_acks_when_both_bpp_and_gateway_signatures_are_valid(mock_record, client):
    bpp_pub, bpp_priv = generate_signing_key_pair()
    gateway_pub, gateway_priv = generate_signing_key_pair()
    payload = _build_on_track_payload()
...
@pytest.mark.django_db
def test_on_track_view_accepts_missing_gateway_authorization_now_that_gateway_is_optional(
    client,
):
    """livetracker4.md §1.2: on_track now arrives directly from the BPP with no
...
@pytest.mark.django_db
def test_on_track_view_rejects_missing_bpp_authorization_even_without_gateway(client):
...
@pytest.mark.django_db
def test_record_on_track_result_stores_the_real_tracking_object():
    _session_with_confirmed_order()
    payload = _build_on_track_payload()
    ...
    assert session.tracking == {"status": "inactive"}
    assert session.tracking_error is None


@pytest.mark.django_db
def test_record_on_track_result_stores_a_real_error_instead_of_a_tracking_object():
    _session_with_confirmed_order()
    payload = _build_on_track_payload(
    ...
    session = SearchSession.objects.get(transaction_id="txn-1")
    assert session.tracking_error == {
        "code": "SLOT_UNAVAILABLE",
        "message": "No matching booking for this order",
    }
    assert session.tracking is None


@pytest.mark.django_db
def test_record_on_track_result_drops_a_callback_for_an_unknown_transaction():
    payload = _build_on_track_payload()
