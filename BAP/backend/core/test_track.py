...
def test_trigger_track_targets_the_same_bpp_and_sends_a_real_signed_order_id(
    ...
    def gateway_track_callback(request):
    ...
    with responses.RequestsMock() as rsps:
        rsps.add_callback(
            responses.POST, "http://gateway:8000/track", callback=gateway_track_callback
        )
        resp = client.post(
...
@pytest.mark.django_db
def test_on_track_view_rejects_missing_gateway_authorization(client):
    bpp_pub, bpp_priv = generate_signing_key_pair()
    payload = _build_on_track_payload()
    body = json.dumps(payload).encode()
    bpp_header = sign_outbound_request(
