...
def test_onboarding_subscribe_marks_under_subscription_before_calling_registry(
    ...
    after a first, insufficient fix): Registry's own `handle_subscribe()` dispatches the
    on_subscribe challenge *synchronously, before responding* — so
    `handle_on_subscribe()`'s own bulk `.filter(status=UNDER_SUBSCRIPTION).update(...)`
    runs *nested inside* `registry_client.subscribe()` below, before it ever returns.
    That filter only matches a row already `UNDER_SUBSCRIPTION` — `submit_subscribe()`
    used to only write that status *after* the call returned, so the nested filter
    found nothing to match, silently confirming nothing despite Registry's own
    challenge succeeding. Simulated here by having the mocked /subscribe callback
    assert the row is already `UNDER_SUBSCRIPTION` at the moment it fires (the real
    precondition `handle_on_subscribe`'s filter depends on) and then perform the exact
    same filtered bulk update `handle_on_subscribe` does — this fails loudly if
