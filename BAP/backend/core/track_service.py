...
def trigger_track(*, transaction_id: str, customer=None) -> None:
    ...
    `customer` (§3.7): IDOR protection — raises `TrackError(401/403)` if this
    ...
    try:
        ...
        raise TrackError(exc.message, status_code=exc.status_code) from exc
    ...
    # livetracker4.md §1.1: direct BAP->BPP dispatch — /track is one of the 9
    ...
    try:
        ...
        raise TrackError(str(exc), status_code=502) from exc

    try:
        response = registry_client.get_bpp_client(session.selected_bpp_id).post(
            bpp_url.rstrip("/") + "/track",
            data=body,
            headers={"Content-Type": "application/json", "Authorization": auth_header},
        )
        response.raise_for_status()
    except Exception as exc:
        raise TrackError(f"BPP unreachable: {exc}", status_code=502) from exc

    ack_status = response.json().get("message", {}).get("ack", {}).get("status")
    if ack_status != "ACK":
        raise TrackError("BPP rejected the track request (NACK)", status_code=502)


def get_track_result(*, transaction_id: str, customer=None) -> dict | None:
...
def validate_and_ack_on_track(
    *, payload: dict, authorization_header: str, gateway_authorization_header: str, body: bytes
    ...
    try:
        # livetracker4.md §1.2: on_track now arrives directly from BPP, with no
