...
def trigger_update(*, transaction_id: str, requested_timestamp: str, customer=None) -> None:
    ...
    # livetracker4.md §1.1: direct BAP->BPP dispatch — /update is one of the 9
    ...
    try:
        ...
        raise UpdateError(str(exc), status_code=502) from exc

    try:
        response = registry_client.get_bpp_client(session.selected_bpp_id).post(
            bpp_url.rstrip("/") + "/update",
            data=body,
            headers={
        ...
        raise UpdateError(f"BPP unreachable: {exc}", status_code=502) from exc

    ack_status = response.json().get("message", {}).get("ack", {}).get("status")
    if ack_status != "ACK":
        raise UpdateError("BPP rejected the update request (NACK)", status_code=502)
...
def validate_and_ack_on_update(
    *, payload: dict, authorization_header: str, gateway_authorization_header: str, body: bytes
    ...
    try:
        # livetracker4.md §1.2: on_update now arrives directly from BPP, with no
