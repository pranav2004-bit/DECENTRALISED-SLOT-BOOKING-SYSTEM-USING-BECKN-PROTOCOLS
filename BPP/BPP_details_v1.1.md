...
> **Implementation note (2026-09-25, `livetracker5.md` Phase 4):** the Payment Module is real on BPP's side today, scoped to **awareness and settlement**, not collection — `livetracker5.md` §0.2 decided BAP collects payment by default for all 3 domains, so BPP never runs a hosted checkout itself. What BPP does have: (1) real payment-status visibility — `Booking.payment_status`, updated by a signed `POST /payment_status` notification from BAP (not a real Beckn action; shaped and verified like one via the same `trust`/signature machinery, since payment status isn't part of the protocol spec), fanned out live to the business Orders dashboard over the existing `resource-{id}-orders` WebSocket group; (2) a real, tested `charge_via_bpp()` call path proving `shared/payment_gateway`'s vendor-agnostic interface genuinely supports BPP-initiated collection too (Design Principle 1), built but not called by any live domain yet — no domain needs `collected_by="bpp"` today; (3) a real refund call triggered by `cancel_service.record_on_cancel_result` against a `SUCCEEDED` payment, covering both BAP- and BPP-initiated cancellations since both converge on the same real `/on_cancel` receipt. All of this is genuinely tested wherever that's possible without live vendor credentials; the real-sandbox proof itself is deferred to a single consolidated live-verification pass once Razorpay sandbox credentials are fully wired in.
