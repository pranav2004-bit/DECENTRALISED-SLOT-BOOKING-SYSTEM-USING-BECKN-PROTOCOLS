...
def validate_and_ack_confirm(
    *, payload: dict, authorization_header: str, gateway_authorization_header: str, body: bytes
    ...
    try:
        # livetracker4.md §1.2: /confirm now arrives directly from BAP, no Gateway
...
def dispatch_on_confirm(*, payload: dict, correlation_id: str | None = None) -> None:
    ...
    # /confirm is one of the 9 post-search actions the real protocol never routes
    # through Gateway once discovery has happened.
    bap_on_confirm_url = context["bap_uri"].rstrip("/") + "/on_confirm"
    try:
        ...
        logger.exception(
            "dispatch_on_confirm: sending on_confirm to %s failed", bap_on_confirm_url
        )
