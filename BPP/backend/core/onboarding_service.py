...
# §2.2. Real reason this exists: Registry requires a re-Subscribe's Authorization header
# to be signed with the CURRENTLY REGISTERED (old) signing key, but its domain-ownership
# verification file must be signed with the NEW signing key being submitted in that same
# request's payload. The identical split exists for encryption: Registry's on_subscribe
# challenge dispatch (synchronous, mid-Subscribe) encrypts the challenge using the NEW
# encryption public key just submitted. Both callbacks arrive as genuinely separate HTTP
# requests — possibly a different gunicorn worker than the one running the rotation
# command — so this can't be a plain in-process variable either. A short TTL
# (comfortably longer than one Subscribe round trip) means an interrupted/crashed
# rotation attempt can't leave either lingering.
_PENDING_ROTATION_SIGNING_CACHE_KEY = "bpp:pending_rotation_signing_key"
_PENDING_ROTATION_ENCRYPTION_CACHE_KEY = "bpp:pending_rotation_encryption_key"
...
def submit_subscribe(
    ...
    *,
    ...
    the caller controls the identity being rotated *to*). Get this wrong and Registry
    returns a clean, expected `401 UNAUTHORIZED` — not a bug, by design."""
