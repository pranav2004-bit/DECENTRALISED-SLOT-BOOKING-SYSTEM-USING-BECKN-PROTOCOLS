...
includes `next_cursor: string | null`). Mirrors `BAP/backend/core/pagination.py`'s own
`parse_limit` exactly (same convention, not a second one invented for this app) — not a
cross-app import, since BAP and BPP are separate Django projects with no shared code
path for app-level modules (only `shared/` packages cross that boundary).
"""
