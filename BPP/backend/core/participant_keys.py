"""BPP's own signing/encryption identity for Beckn network participation
...
never (re)generated. Mirrors BAP/backend/core/participant_keys.py — see that module's
docstring for why "ephemeral" (Registry's own pattern) isn't viable for onboarding.

No `@lru_cache` on `get_signing_keys`/`get_encryption_keys` (removed, mirroring the fix
applied to Registry's `registry_keys.py` §1.2 and Gateway's `participant_keys.py`
§2.2) — the same bug applies here: BPP runs multiple gunicorn workers, and an
in-process cache means a real rotation writes the new key to disk but every
already-running worker keeps serving the *old* key from memory until restarted. The old
`rotate_signing_key()`/`rotate_encryption_key()` called `get_signing_keys.cache_clear()`
after writing, but that only clears the cache in whatever process called it (the
`onboarding_rotate_keys` management command — a separate, short-lived process, not any
of the live gunicorn workers actually serving traffic) — it never touched the workers'
own stale caches. Not a hot path here either (participant-key reads happen at
Subscribe/dispatch time, not every request), so re-reading a small JSON file per call is
cheap and correctness-first.
"""
...
# below, for the ENTIRE function body including the read path — `path.write_text()`
# isn't atomic (open/truncate, write, close), so a check-and-read *outside* the lock is
# a real race, observing another thread's file mid-write. This function is called at
...
def rotate_signing_key() -> tuple[str, str]:
    ...
    logger.warning("Rotated BPP signing key (previous key backed up to %s).", backup_path)
    return public_key, private_key
...
def rotate_encryption_key() -> tuple[str, str]:
    ...
    logger.warning("Rotated BPP encryption key (previous key backed up to %s).", backup_path)
    return public_key, private_key
...
def restore_keys_from_backup(backup_paths: dict[str, str]) -> None:
    """Rolls BPP's own identity back to what a prior `backup_current_keys()` call saved
    — used when a rotation's re-Subscribe step fails, so BPP doesn't end up signing with
    a key Registry never actually learned about."""
    ...
    logger.warning("Restored BPP's signing+encryption keys from backup after a failed rotation.")
