...
def validate_and_ack_support(
    *, payload: dict, authorization_header: str, gateway_authorization_header: str, body: bytes
    ...
    try:
        # livetracker4.md §1.2: /support now arrives directly from BAP, no Gateway
...
def dispatch_on_support(*, payload: dict) -> None:
    ...
    # /support is one of the 9 post-search actions the real protocol never routes
    # through Gateway once discovery has happened.
    bap_on_support_url = context["bap_uri"].rstrip("/") + "/on_support"
    try:
        ...
        logger.exception(
            "dispatch_on_support: sending on_support to %s failed", bap_on_support_url
        )
