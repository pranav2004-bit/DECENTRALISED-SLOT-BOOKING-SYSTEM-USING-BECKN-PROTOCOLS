"""Phase 3.5 Test Gate (livetracker2.md §3.5) pieces owned by BPP: real /status
receipt (verifying both the BAP and the forwarding Gateway) and real /on_status
dispatch — a read-only lookup of a real confirmed booking's live current state,
including its real fulfillment_status, and the IDOR-shaped holder_ref-mismatch
rejection.
"""
...
from core import status_service
...
def _make_active_booking(*, holder_ref="txn-1", price_value="899.00"):
...
def _build_status_payload(*, order_id, bap_id="bap.example.com", transaction_id="txn-1"):
    return {
        "context": {
            "domain": "ONDC:RET13",
...
def test_status_view_acks_when_both_bap_and_gateway_signatures_are_valid(
    bpp_identity_settings, client
):
    bap_pub, bap_priv = generate_signing_key_pair()
    gateway_pub, gateway_priv = generate_signing_key_pair()
    payload = _build_status_payload(order_id="11111111-1111-1111-1111-111111111111")
    ...
    with (
        patch("core.status_service.dispatch_on_status_in_background") as mock_dispatch,
...
@pytest.mark.django_db
def test_status_view_accepts_missing_gateway_authorization_now_that_gateway_is_optional(client):
    """livetracker4.md §1.2: /status now arrives directly from the BAP with no
    ...
    with (
        patch("core.status_service.dispatch_on_status_in_background"),
        responses.RequestsMock() as rsps,
    ):
        rsps.add_callback(
            responses.POST, "http://registry:8000/lookup", callback=_lookup_callback(known)
...
@pytest.mark.django_db
def test_status_view_rejects_missing_bap_authorization_even_without_gateway(client):
...
@pytest.mark.django_db
def test_status_view_rejects_a_missing_order_id_before_acking(client):
    bap_pub, bap_priv = generate_signing_key_pair()
    gateway_pub, gateway_priv = generate_signing_key_pair()
    payload = {"context": _build_status_payload(order_id="x")["context"], "message": {}}
    body = json.dumps(payload).encode()
    bap_header = sign_outbound_request(
        body=body,
        subscriber_id="bap.example.com",
...
@pytest.mark.django_db
def test_dispatch_on_status_returns_the_real_live_order_state(bpp_identity_settings):
    resource, slot, booking = _make_active_booking(holder_ref="txn-1")
    payload = _build_status_payload(order_id=booking.id, transaction_id="txn-1")
    ...
    assert forwarded["context"]["action"] == "on_status"
    assert "error" not in forwarded
    order = forwarded["message"]["order"]
    assert order["id"] == str(booking.id)
    assert order["status"] == "ACTIVE"
    assert order["fulfillments"][0]["state"]["descriptor"]["code"] == "SCHEDULED"


@pytest.mark.django_db
def test_dispatch_on_status_does_not_raise_when_bap_is_unreachable(bpp_identity_settings):
    """livetracker4.md §1.4 coverage-parity replacement for beckn-gateway's retired
    test_relay_on_status_does_not_raise_when_bap_is_unreachable."""
    resource, slot, booking = _make_active_booking(holder_ref="txn-1")
    payload = _build_status_payload(order_id=booking.id, transaction_id="txn-1")
...
@pytest.mark.django_db
def test_dispatch_on_status_rejects_a_booking_held_by_a_different_transaction(
    bpp_identity_settings,
):
    resource, slot, booking = _make_active_booking(holder_ref="txn-owner")
    payload = _build_status_payload(order_id=booking.id, transaction_id="txn-attacker")
...
@pytest.mark.django_db
def test_dispatch_on_status_rejects_an_unknown_order_id(bpp_identity_settings):
