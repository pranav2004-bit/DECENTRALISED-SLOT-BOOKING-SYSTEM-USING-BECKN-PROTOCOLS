...
def validate_and_ack_update(
    *, payload: dict, authorization_header: str, gateway_authorization_header: str, body: bytes
    ...
    try:
        # livetracker4.md §1.2: /update now arrives directly from BAP, no Gateway
...
def dispatch_on_update(*, payload: dict, correlation_id: str | None = None) -> None:
    ...
    # /update is one of the 9 post-search actions the real protocol never routes
    # through Gateway once discovery has happened.
    bap_on_update_url = context["bap_uri"].rstrip("/") + "/on_update"
    try:
        ...
        logger.exception(
            "dispatch_on_update: sending on_update to %s failed", bap_on_update_url
        )
