...
**Test/dev Redis-DB isolation (real gap found and closed, `livetracker2.md` §4.7, 2026-07-29):** these counters and the live dev server's sessions used to share the exact same Redis DB (`bap-cache:6379/0`/`bpp-cache:6379/0`) that `conftest.py`'s autouse `_clear_rate_limit_cache` fixture wipes before/after every test — running `pytest` against the running `docker compose` stack silently zeroed real dashboard counters and logged out every real session. Fixed in both apps' `settings.py`: `CACHES["default"]["LOCATION"]` resolves to Redis DB 15 when `TESTING = "pytest" in sys.modules`, real DB 0 otherwise — tests keep a real Redis-backed cache (preserving the atomic-`INCR` guarantee this counters implementation depends on), just isolated from the dev server. See `RUNBOOK.md`'s matching entry for the full live-reproduction evidence.
...
## Real-Time (WebSocket) Observability — real gap found, not yet closed (`livetracker3.md` §5.1)

`shared/realtime/consumers.py`'s `FoundationConsumer` and BPP's own `ResourceAvailabilityConsumer` (`livetracker2.md` §4.4, the real live-inventory-push feature) currently have **zero structured logging** — no `connect`/`disconnect`/`4401`/`4403`-close-code log lines, no `correlation_id` propagation onto the WebSocket handshake, and no metric counting active connections or messages, despite this doc's own "Structured Logging" section describing a blanket standard ("every log line"). Found by direct audit, honestly recorded rather than silently glossed over: this is a real code gap, not a documentation-only staleness issue, and fixing it is out of scope for a documentation freshness sweep — it needs its own `[MVP]`/`[PILOT]`-tagged tracker item (logging on `connect`/`disconnect`/authorization-rejection, at minimum) before this section can honestly describe real behavior instead of an aspiration. Revisit as a concrete follow-up item, not fixed here.
