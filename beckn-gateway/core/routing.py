...
livetracker4.md §1.4 (2026-07-31): this module used to also carry the equivalent
validate_and_ack_X / dispatch_X / relay_on_X trio for select, init, confirm, status,
cancel, update, track, rating, and support — retired here. Per the real Beckn protocol
(protocol_compliance_notes_v1.1.md §P), only /search is meant to route through the
Gateway; discovery-complete actions go directly BAP<->BPP once the customer already
knows which BPP they're dealing with. §1.1-1.3 moved that direct dispatch/reception
into BAP's and BPP's own service modules (resolve_subscribed_bpp + get_bpp_client on
the BAP side, get_bap_client on the BPP side) and proved it live before this file's own
dead code was removed. `_verify_caller` stays — /search and /on_search still use it.
"""
